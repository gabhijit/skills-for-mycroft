...
class BusSkill(MycroftSkill):
    ...
    def handle_bus_travel_intent(self, message):
    	...
    	date = date.split(" ")[0] 
    	url = "http://developer.goibibo.com/api/bus/search/?app_id=<YourAppId>&app_key=<YourAppKey>&format=json&source="+source+"&destination="+destination+"&dateofdeparture="+date
    	response = requests.get(url).json()
	for i in response['data']['onwardflights']:
		skey = i['skey']
		encoded_skey = urllib.quote_plus(skey)
		busseat_url = "http://developer.goibibo.com/api/bus/seatmap/?app_id=<YourAppId>&app_key=<YourAppKey>&skey=" + encoded_skey + "\""
