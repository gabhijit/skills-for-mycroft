...
class RailSkill(MycroftSkill):
    ...
    def handle_rail_travel_intent(self, message):
   	...
   	url = "https://api.railwayapi.com/v2/suggest-station/name/" + source + "/apikey/ppipz4ugxv/"
   	response = requests.get(url).json()
   	for i in response['stations']:
   		self.speak("station name is "+ (i['name']))
   		self.speak("station code is "+ (i['code']))
  	...
  	self.speak("destination is "+ destination)
   	url = "https://api.railwayapi.com/v2/suggest-station/name/" + destination + "/apikey/ppipz4ugxv/"
   	response = requests.get(url).json()
   	for i in response['stations']:
   		self.speak("station name is "+ (i['name']))
   		self.speak("station code is "+ (i['code']))
	...
	url = "https://api.railwayapi.com/v2/between/source/"+res+"/dest/"+res1+"/date/"+date+"/apikey/ppipz4ugxv/"
